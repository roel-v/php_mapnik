...
{    
    if (argc != 2)
    {
        ...
        std::cout << "Warning: ./rundemo looks for data in demo/data/,\nTherefore must be run from within the root folder.\n";
        return EXIT_SUCCESS;
    }
    ...
    try {
        ...
        std::cout << " looking for 'shape.input' plugin in... " << mapnik_dir << "/plugins/" << "\n";
        datasource_cache::instance()->register_datasources(mapnik_dir + "/plugins/"); 
