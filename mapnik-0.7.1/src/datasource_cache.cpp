...
namespace mapnik
{
...
#ifdef WIN32
           (create_ds*) GetProcAddress(itr->second->handle(), "create");
#else
   ...
   bool datasource_cache::insert(const std::string& type,const PluginInfo::TDynamicLibraryHandle module)
   {
      ...
      if (exists(path) && is_directory(path))
      {
         for (filesystem::directory_iterator itr(path);itr!=end_itr;++itr )
         {
...

            {
               try 
               {
                  PluginInfo::TDynamicLibraryHandle module=
#ifdef WIN32
					  ::LoadLibrary(itr->string().c_str());
#else
					  lt_dlopen(itr->string().c_str());
#endif
                  if (module)
                  {
                     datasource_name* ds_name = 
#ifdef WIN32
                        (datasource_name*) GetProcAddress(module, "datasource_name");
#else
                  ...
                  {
                     std::clog << "Problem loading plugin library: " << itr->string().c_str() << " (libtool error: " << 
#ifdef WIN32
						 ""
#else
						 lt_dlerror() 
